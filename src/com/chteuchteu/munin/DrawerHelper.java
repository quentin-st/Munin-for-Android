	...
	private void initDrawer() {
		...
		if (!m.premium) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
				a.findViewById(R.id.drawer_notifications_img).setAlpha(0.6f);
				a.findViewById(R.id.drawer_notifications_txt).setAlpha(0.6f);
			}
		...
		for (final MuninPlugin mp : m.currentServer.getPlugins()) {
			...
			b.setOnClickListener(new OnClickListener() {
				public void onClick (View v) {
					...
					int p = 0;
					for (int i=0; i<m.currentServer.getNbPlugins(); i++) {
