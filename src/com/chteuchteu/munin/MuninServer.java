...
public class MuninServer extends Model {
	...
	public MuninServer() {
		...
		this.name = "";
		this.serverUrl = "";
		this.pluginsList = new ArrayList<MuninPlugin>();
	...
	public void fetchPluginsList() {
		...
		//MuninPlugin[] mp = new MuninPlugin[300];
		...
		if (html == null || html == "" || html == "error") {
			...
			int nbPlugins;	MuninPlugin currentPl;
			...
			this.numberOfPlugins = nbPlugins;
			
			/*this.pluginsList = new ArrayList<MuninPlugin>();
			for (int i=0; i<mp.length; i++) {
				if (mp[i] != null)
					this.pluginsList.add(mp[i]);
			}*/
			this.pluginsList = mp;
		}
	}
	public void fetchPluginsStates() {
		...
		if (html != "") {
			...
			for (Element image : images) {
				...
				for (MuninPlugin m : this.pluginsList) {
		...
		for (int i=0; i<this.getNbPlugins(); i++) {
	...
	public void importData(MuninServer source) {
		...
		this.pluginsList = source.pluginsList;
		this.authNeeded = source.authNeeded;
		this.authLogin = source.authLogin;
		this.authPassword = source.authPassword;
		this.authType = source.authType;
		this.numberOfPlugins = source.numberOfPlugins;
		...
		try {
			...
			if (authNeeded) {
				if (this.getAuthType() == AUTH_BASIC)
					...
					// Parser le header
					realmName = Util.match(this.authString, "realm");
					nonce = Util.match(this.authString, "nonce");
					opaque = Util.match(this.authString, "opaque");
					qop = Util.match(this.authString, "qop");
	...
	public MuninPlugin getPlugin(int pos) {
		if (pos < this.pluginsList.size() && pos >= 0)
			return this.pluginsList.get(pos);
		else
			return null;
	}
	...
	public void setAuthNeeded(boolean b) {
		this.authNeeded = b;
	}
