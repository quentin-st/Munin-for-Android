...
public class SQLite {
	...
	public void saveMuninLabels() {
		...
		for (MuninLabel l : muninFoo.labels) {
			for (MuninPlugin p : l.plugins) {
				MuninLabelRelation rel = new MuninLabelRelation(getBDDInstance(p, p.getInstalledOn()), l.getName());
				Log.v("", "Saving label " + l.getName() + " \t\t " + p.getName());
				rel.save();
			}
		}
		Log.v("", "=====================================");
	...
	public void logServersTable() {
		...
		logLine(60);
		for (MuninServer s : getServers()) {
			String[] fields = {s.getName(), s.getNbPlugins() + "", s.getPosition() + "" };
			int[] space = { 33, 9, 8 };
